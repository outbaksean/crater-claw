...
- `SubAgentPlugin` — invokable child agent. Reads `PluginExecutionContext.CurrentEndpoint` and `CurrentDepth` at call time. Resolves the child `BehaviorProfile` by `profileId`, resolves child plugins via `IPluginRegistry`, constructs an `AgenticRequest` with `Depth + 1`, and calls `IAgenticExecutionService.ExecuteAsync`. Returns the child response content as a string, or an error string if the endpoint is null or the profile is not found.
- `PluginExecutionContext` — static class with `AsyncLocal<ProviderEndpoint?> CurrentEndpoint` and `AsyncLocal<int> CurrentDepth`. Set by `SemanticKernelAgenticExecutionService` at the start of each `ExecuteAsync` call. Flows through async continuations into plugin calls.
